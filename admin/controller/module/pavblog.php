...
class ControllerModulePavblog extends Controller {
	...
	public function setTemplate( $tpl ){
		...
		$template = 'module/pavblog/'.$tpl.".tpl";
	...
	public function category(){
        ...
        $this->mdata['ckeditor'] = $this->config->get('config_editor_default');
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$template = 'module/pavblog/category_form.tpl';
		$this->response->setOutput($this->load->view($template, $this->mdata));	
	}
	...
	public function blog(){
		...
		foreach( $this->mdata['languages'] as $k => $language ){
			if( isset($data['blog_description']) && isset($data['blog_description'][$language['language_id']]) ){
				...
				$blog_descriptions[$language['language_id']] = array('title'=>'','lang_tags'=>'','description'=>'','content'=>'');
			}
		}

		// RelatedProduct
		if (isset($this->request->post['action_mode'])) {
			$products = $this->request->post['product_related'];
		} elseif (isset($blog['blog_id'])) {
			$products = $this->model_pavblog_blog->getBlogRelatedProduct($blog['blog_id']);
		} else {
			$products = array();
		}
