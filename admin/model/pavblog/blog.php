...
class ModelPavblogblog extends Model { 
	...
	public function saveData( $data ){
		...
		if( isset($data["pavblog_blog_description"]) ){ 
			...
			foreach( $languages as $language ){
				$sql = "INSERT INTO ".DB_PREFIX ."pavblog_blog_description(`language_id`, `blog_id`,`title`,`description`,`content`,`lang_tags`) 
							...
							.$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['description'])."','".$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['content'])."','".$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['lang_tags'])."') ";
		...
		if (isset($data['product_related'])) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "pavblog_related_product WHERE blog_id = '" . (int)$data['pavblog_blog']['blog_id'] . "'");
			
			foreach ($data['product_related'] as $related_id) {	
				$this->db->query("INSERT INTO " . DB_PREFIX . "pavblog_related_product SET blog_id = '" . (int)$data['pavblog_blog']['blog_id'] . "', product_id = '" . (int)$related_id . "'");
			}
	...
	public function getBlogRelatedProduct($id) {
		$pavblog_related_product = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "pavblog_related_product WHERE blog_id = '" . (int)$id . "'");
		
		foreach ($query->rows as $result) {
			$pavblog_related_product[] = $result['product_id'];
		}
		
		return $pavblog_related_product;
	}
